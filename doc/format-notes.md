...
**Not corpus-confirmed:** every vendored image is single-plane
(`planeCount == 1`, `planeOnOff == 0`), so the multi-plane concatenation
order (planes stored sequentially per the prose's "1st plane / optional 2nd
plane / ..." enumeration, plane 0 contributing the pixel index's low bit) and
the "present vs. filled vs. neither" combination are implemented literally
from the prose spec and exercised only by hand-written synthetic tests, not
by any real file. Deferred to the phase-11 wild sweep, same treatment as
phase 5's dithermask.
